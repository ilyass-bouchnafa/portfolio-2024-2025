...
char* afficherEntier(Objet* objet) {
    int* entier = (int*) objet;
    char* output = (char*)malloc(sizeof(int));
    snprintf(output, sizeof(int), "%d",*entier);
    return output;
}
...
void detruireListe(Liste* li) {
    ...
    initListe(li,0,afficherEntier,comparerInt);
}

int chercherMax(Liste *li) {
    ouvrirListe(li);
    int *max = (int*)objetCourant(li);
    while(!finListe(li)) {
        int *ptc = (int*)objetCourant(li);
        if (*max < *ptc)
            max = ptc;
    }
    return *max;
